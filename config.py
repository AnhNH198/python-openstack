class query:
    # query cpu
    # url = "https://url:3000/api/datasources/proxy/4/query"
    # params = {
    #     "db":"publicthor",
    #     "q": {
    #         "param":"value",
    ...
    def query_ram(sid, name, type=None):
        query = f'https://url:3000/api/datasources/proxy/4/query?db=publicthor&q=SELECT mean(\"value\") FROM \"instances\" WHERE (\"tags1\" =~ /^{sid}$/ AND \"tags2\" = \'libvirt-kvm\' AND \"tags3\" = \'{name}\' AND "tags4" = \'free\') AND time >= now() - 24h GROUP BY time(2m) fill(none)&epoch=ms'
        return query
    
    # query disk size
    def query_disk_size(sid, name, type=None):
        query = f'https://url:3000/api/datasources/proxy/4/query?db=publicthor&q=SELECT mean(\"value\") FROM \"instances\" WHERE (\"tags1\" =~ /^{sid}$/ AND "tags2" = \'libvirt-kvm\' AND \"tags3\" = \'{name}\' AND \"tags5\" = \'used-percent\') AND time >= now() - 24h GROUP BY time(2m), "tags4" fill(none)&epoch=ms'
        return query

    # query block read/write_bytes | iops
    def query_block_rw_iops(sid, name, type):
        query = f'https://url:3000/api/datasources/proxy/4/query?db=publicthor&q=SELECT mean(\"value\") FROM \"instances\" WHERE (\"tags1\" =~ /^{sid}$/ AND \"tags2\" = \'libvirt-kvm\' AND "tags3" = \'{name}\' AND \"tags4\" =~ /^(vda|vdb|vdc|vdd|vde|vdf|vdg|vdh|vdi)$/ AND \"tags5\" = \'{type}\') AND time >= now() - 24h GROUP BY time(2m), \"tags4\" fill(none)&epoch=ms'
        return query

    # Query net tx/rx_packets
    def query_net_tx_rx_pkt(sid, name, type):
        query = f'https://url:3000/api/datasources/proxy/4/query?db=publicthor&q=SELECT mean(\"value\") FROM \"instances\" WHERE (\"tags1\" =~ /^{sid}$/ AND \"tags2\" = \'libvirt-kvm\' AND "tags3" = \'{name}\' AND "tags4" =~ /^(eth0|eth1|eth2|eth3|eth4|eth5|eth6|eth7|eth8)$/ AND "tags5" = \'{type}\') AND time >= now() - 24h GROUP BY time(2m), \"tags4\", \"tags5\" fill(none)&epoch=ms'
        return query

    # Query net tx/rx_bytes
    def query_net_tx_rx_bytes(sid, name, type):
        query = f'https://url:3000/api/datasources/proxy/4/query?db=publicthor&q=SELECT mean("value") * 8 FROM "instances" WHERE ("tags1" =~ /^{sid}$/ AND "tags2" = \'libvirt-kvm\' AND "tags3" = \'{name}\' AND "tags4" =~ /^(eth0|eth1|eth2|eth3|eth4|eth5|eth6|eth7|eth8)$/ AND "tags5" = \'{type}\') AND time >= 1613602671298ms and time <= 1613604038387ms GROUP BY time(30s), "tags4" fill(none)&epoch=ms'
        return query
    def query_prometheus(sid, name=None, type=None):
        query = f'https://url:3000/api/datasources/proxy/18/api/v1/query_range?query=sum(node_cpu_info{{instance_id="{sid}"}})&start=1613688255&end=1613699055&step=120'

class queryLbs:
    def req_per_sec_frontend(lbid=None, frontend=None, backend_id=None, timefilter=None):
        query = f'https://url:3000/api/datasources/proxy/3/query?db=octavia&q=SELECT SUM("derivative") FROM ( SELECT derivative(mean("value"), 30s) FROM "servers" WHERE ("tags3" = \'haproxy\' AND "tags4" =~ /^{lbid}$/ AND "tags5" = \'frontend\' AND "tags6" =~ /^{frontend}$/ AND "tags7" = \'total\' AND "tags8" = \'req_tot\') AND time >= now() - {timefilter} GROUP BY time(30s), "tags4", "tags6", "tags7", "tags2" )  GROUP BY time(30s)&epoch=ms'
        return query
    ...
    def error_connection_per_second_backend(lbid=None, frontend=None, backend_id=None, timefilter=None, backend_server="All"):
        query = f'https://url:3000/api/datasources/proxy/3/query?db=octavia&q=SELECT mean("value") FROM "servers" WHERE ("tags1" =~ /^{lbid}$/ AND "tags3" = \'haproxy\' AND "tags5" = \'backend\' AND "tags6" =~ /^{backend_id}$/ AND "tags7" =~ /^{backend_server}$/ AND "tags8" = \'econ\') AND time >= now() - {timefilter} GROUP BY time(30s), "tags7" fill(null);&epoch=ms'
        return query
    def average_backend_response_time(lbid=None, frontend=None, backend_id=None, timefilter=None, backend_server="All"):
        query = f'https://url:3000/api/datasources/proxy/3/query?db=octavia&q=SELECT mean("value") FROM "servers" WHERE ("tags1" =~ /^{lbid}$/ AND "tags3" = \'haproxy\' AND "tags5" = \'backend\' AND "tags6" =~ /^{backend_id}$/ AND "tags7" =~ /^{backend_server}$/ AND "tags8" = \'rtime\') AND time >= now() - {timefilter} GROUP BY time(30s), "tags7" fill(null);&epoch=ms'
        return query
    def error_response_rate_backend(lbid=None, frontend=None, backend_id=None, timefilter=None):
        query = f'https://url:3000/api/datasources/proxy/3/query?db=octavia&q=SELECT mean("value") FROM "servers" WHERE ("tags1" =~ /^{lbid}$/ AND "tags3" = \'haproxy\' AND "tags5" = \'backend\' AND "tags6" =~ /^{backend_id}$/ AND "tags7" =~ /^()$/ AND "tags8" = \'eresp\') AND time >= now() - {timefilter} GROUP BY time(30s), "tags7" fill(null) OFFSET 1;&epoch=ms'
        return query
    def rate_session_per_second_backend(lbid=None, frontend=None, backend_id=None, timefilter=None):
        query = f'https://url:3000/api/datasources/proxy/3/query?db=octavia&q=SELECT mean("value") FROM "servers" WHERE ("tags1" =~ /^{lbid}$/ AND "tags3" = \'haproxy\' AND "tags5" = \'backend\' AND "tags6" =~ /^{backend_id}$/ AND "tags7" =~ /^()$/ AND "tags8" = \'rate\') AND time >= now() - {timefilter} GROUP BY time(30s), "tags7" fill(null);&epoch=ms'
        return query